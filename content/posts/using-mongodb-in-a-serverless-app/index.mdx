...
If you have been developing an API in Node setting up a connection to a MongoDB database is relatively straight forward. Check out my [previous post](https://akhilaariyachandra.com/setup-mongo-db-in-node-js-with-mongoose) if you need a refresher.
...
```typescript:title=src/database/index.ts {7-18}
import { Connection, createConnection } from "mongoose";
...
```typescript:title=src/database/models/note.ts {10-14}
import {
  Document,
  SchemaDefinition,
  SchemaTypes,
...
```typescript:title=src/database/models/note.ts {16-23}
import {
  Document,
  SchemaDefinition,
  SchemaTypes,
...
```typescript:title=src/database/models/note.ts {25-28}
import {
  Document,
  SchemaDefinition,
  SchemaTypes,
