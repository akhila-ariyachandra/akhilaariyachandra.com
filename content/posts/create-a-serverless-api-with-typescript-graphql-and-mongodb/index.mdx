...
In the _src_ folder create the _grpahql_ folder and in that create the _schema_ folder.
...
First let's write the resolver for **getAllNotes**. Start be declaring the function.
...
```typescript:title=src/graphql/resolvers/note.ts {12-25}
import mongoose from "mongoose";
import dayjs from "dayjs";
import NoteModel, { INote } from "../../database/models/note";
import { ApolloError } from "apollo-server-micro";
...
If you want a more detailed explaination into GraphQL and making a server for it, you can follow the excellent guide that I learned from [here](https://www.robinwieruch.de/graphql-apollo-server-tutorial).
