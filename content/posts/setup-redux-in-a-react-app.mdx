...
banner: "/blog/setup-redux-in-a-react-app/banner.png"
---
...
```javascript:src/redux/user/initialState.js
const initialState = {
...
```javascript:src/redux/user/constants.js
const constants = {
...
```javascript:src/redux/user/actions.js
...
```javascript:src/redux/user/reducer.js
import constants from "./constants";
...
<PostImage
  src="/blog/setup-redux-in-a-react-app/app_preview.png"
  width={209}
  height={129}
  title="App Preview"
/>
...
```javascript:src/components/Display.js
...
We can set the component to load the user every time the `userId` in the redux state changes. If you want to learn about how to mimic react life cycle methods with hooks, check my post [here](https://akhilaariyachandra.com/mimic-react-life-cycles-methods-with-hooks/).
...
```jsx:src/components/Controls.js
import React from "react";
import { connect } from "react-redux";
import { bindActionCreators } from "redux";
