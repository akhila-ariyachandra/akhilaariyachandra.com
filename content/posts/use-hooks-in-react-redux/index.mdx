...
```jsx:title=src/components/Controls.js {2}
import React from "react";
...
```jsx:title=src/components/Controls.js {6}
import React from "react";
import { useSelector, useDispatch } from "react-redux";
import { setUserId } from "../redux/user/actions";
...
```jsx:title=src/components/Controls.js {7}
import React from "react";
import { useSelector, useDispatch } from "react-redux";
import { setUserId } from "../redux/user/actions";
...
Just change all actions to run inside the `dispatch` function.

```jsx:title=src/components/Controls.js {12,19}
import React from "react";
import { useSelector, useDispatch } from "react-redux";
import { setUserId } from "../redux/user/actions";
...
1. Remove all the `connect()` Higher Order Component related code.
2. Import `useSelector` and `useDispatch`.

```javascript noLineNumbers
import { useSelector, useDispatch } from "react-redux";
```

3. Declare the user state using `useSelector`.

```javascript noLineNumbers
const user = useSelector(state => state.user);
```

4. Declare the dispatch function using `useDispatch`.

```javascript noLineNumbers
const dispatch = useDispatch();
```

5. Change all Redux action to be called inside the `dispatch` function.
...
```jsx:title=src/components/Dsiplay.js {2,5,6-7,10-11,19,24,29,34,42}
import React from "react";
import { useSelector, useDispatch } from "react-redux";
...
<iframe
  src="https://codesandbox.io/embed/react-redux-hooks-starter-51wxq?fontsize=14"
