...
<SpecialBlock
  type="info"
  markdown={`The example above is running with the cache disabled and the "Slow 3G" preset.`}
/>
...
After that we need to get access to the `router` object with the `useRouter` [hook](https://nextjs.org/docs/api-reference/next/router#userouter "useRouter").

```jsx:pages/_app.js {2,12}
import ProgressBar from "@badrap/bar-of-progress";
