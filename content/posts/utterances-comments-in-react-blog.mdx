...
<SpecialBlock
  markdown={`Replace the value for *repo* with your own repository.`}
  type="info"
/>
...
Finally as a bonus we can improve the loading speed of the script by [preloading](https://developer.mozilla.org/en-US/docs/Web/HTML/Link_types/preload) it. To this all we need to do is add a `<link>` tag the `<head>` tag with `rel="preload"`.
...
You can check how I've implemented it my blog [here](https://github.com/akhila-ariyachandra/akhilaariyachandra.com/blob/main/components/post/Comments.tsx).
