...
date: "2019-10-25"
description: "A guide to quickly get Redux up and running in React"
banner: "/setup-redux-in-a-react-app/banner.png"
---
...
For this post I'm going to use the React app I made in an earlier [blog post](https://akhilaariyachandra.com/getting-started-in-react-with-parcel-js/) available [here](https://github.com/akhila-ariyachandra/react-parcel-starter).
...
Before we start setting up the Redux parts, let's create a folder called _redux_ in the _src_ folder to store all our Redux related code.
...
The **reducer** is the part of redux that changes the state based on the object return from the actions. The reducer has two arguments, _state_ for the state to change and _action_ for the object returned by the actions. The initial state will also be set as the default parameter of the _state_ argument.

In the reducer all that has to be done is for the state to be changed based on the action, so we check the _type_ of the action and change the state with the _payload_ of the action.
...
Now that we've setup the initial state, actions and reducers, it's time to tie them all together. First create _index.js_ in _src/redux_ and import the required dependencies.
...
In the end _index.js_ should be like this.
...
![App Preview](/setup-redux-in-a-react-app/app_preview.png)
...
We'll start with the Display component. Create a folder called _components_ in _src_ and then create _Display.js_ in it. Once that's done declare the component in it.
...
Now we can connect redux to it. We'll need the user state and the `getUser` action. We can use the `connect` import from react-redux to wrap the component with a Higher Order Component which will provide the redux state and actions. _connect_ takes two arguments.
...
All we are doing here is accessing the user section of the redux state and sending it through the _user_ prop. The name of the key is the same as the name of the prop.

Before we declare _mapDispatchToProps_ we need two more imports.
...
Then when exporting the component it'll be wrapped in the _connect_ Higher Order Component.
...
We can set the component to load the user every time the _userId_ in the redux state changes. If you want to learn about how to mimic react life cycle methods with hooks, check my post [here](https://akhilaariyachandra.com/mimic-react-life-cycles-methods-with-hooks/).
...
Finally the _Display_ component should be like this.
...
- Instead of importing and using _getUser_ we're using _setUserId_
- We're limiting the _userId_ between 1 and 10 because that's the number of the user records the API has.
- We're also disabling the button based on _isFetchingUser_. It will be set to true when _getUser_ is called so the buttons will get disabled when a request to get the user is made and set to false once it's complete.

## Bringing everything together in the root component

One thing we need to do to enable _react-redux_ throughout the whole app is wrap the root component with `Provider` component from _react-redux_. Once we do that all the child components will be able to use redux through `connect`.
...
_store_ is the redux store will initialize and exported in _src/redux/index.js_.
...
<iframe src="https://codesandbox.io/embed/react-redux-starter-nn8sy?fontsize=14&hidenavigation=1&theme=dark"
