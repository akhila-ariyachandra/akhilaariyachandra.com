...
test.describe("Archived Blog page (/blog/archive)", () => {
  test("is instant on an initial page load", async ({ page, baseURL }) => {
    await instant(
      page,
      async () => {
        ...
        await expect(
          page.getByRole("heading", { level: 1, name: "Archived Blog" }),
        ).toBeVisible();
      },
      {
        baseURL,
  ...
  test("is instant on a client navigation", async ({ page }) => {
    ...
    await instant(page, async () => {
      ...
      await page.waitForURL((url) => url.pathname === "/blog/archive");
      await expect(
        page.getByRole("heading", { level: 1, name: "Archived Blog" }),
      ).toBeVisible();
    });
...
test.describe("Archived Blog post page (/blog/archive/[slug])", () => {
  test("is instant on an initial page load", async ({ page, baseURL }) => {
    await page.goto("/blog/archive");
